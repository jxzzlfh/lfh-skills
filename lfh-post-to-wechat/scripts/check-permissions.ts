...
async function checkApiCredentials(): Promise<void> {
  ...
  const projectEnv = path.join(cwd, '.lfh-skills', '.env');
  const userEnv = path.join(os.homedir(), '.lfh-skills', '.env');
...
async function main(): Promise<void> {
  console.log('=== lfh-post-to-wechat: Permission & Environment Check ===\n');
