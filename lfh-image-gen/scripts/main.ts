...
Options:
  ...
  --provider google|openai|dashscope|volcengine  Force provider (auto-detect by default)
  -m, --model <id>          Model ID
...
Environment variables:
  ...
  VOLCENGINE_API_KEY        VolcEngine API key (火山引擎豆包)
  OPENAI_IMAGE_MODEL        Default OpenAI model (gpt-image-1.5)
  GOOGLE_IMAGE_MODEL        Default Google model (gemini-3-pro-image-preview)
  DASHSCOPE_IMAGE_MODEL     Default DashScope model (z-image-turbo)
  VOLCENGINE_IMAGE_MODEL    Default VolcEngine model (doubao-seedream-5-0-260128)
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--provider") {
      const v = argv[++i];
      if (v !== "google" && v !== "openai" && v !== "dashscope" && v !== "volcengine") throw new Error(`Invalid provider: ${v}`);
      out.provider = v;
      continue;
    }
  ...
  for (const line of lines) {
    ...
    if (trimmed.includes(":") && !trimmed.startsWith("-")) {
      ...
      if (key === "version") {
        ...
        config.default_model = { google: null, openai: null, dashscope: null, volcengine: null };
        currentKey = "default_model";
      } else if (currentKey === "default_model" && (key === "google" || key === "openai" || key === "dashscope" || key === "volcengine")) {
        const cleaned = value.replace(/['"]/g, "");
...
function detectProvider(args: CliArgs): Provider {
  ...
  const available = [hasGoogle && "google", hasOpenai && "openai", hasDashscope && "dashscope", hasVolcengine && "volcengine"].filter(Boolean) as Provider[];
